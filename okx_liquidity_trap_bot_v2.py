...
CONFIG = {
    ...
    "FEE_DRAG_MAX": 0.08,          # комиссия ХУДШЕГО случая ≤ 8% суммы риска
...
VERSION = "3.11"   # единая версия: приветствие, /start и логи берут отсюда
...
def build_trade_plan(signal: dict, balance: float) -> dict:
    ...
    # не превышает FEE_DRAG_MAX от суммы риска). Комиссия считается с полного
    # оборота позиции (вход + выходы) ≈ 2× notional независимо от исхода —
    # поэтому доля риска, съедаемая комиссией, ЗАВИСИТ от ширины стопа:
    # fee_to_risk = 2×FEE_RATE / stop_pct. Узкий стоп = гарантированный
    # фиксированный убыток на любой сделке, даже с R=0 (see 09.07: 58% сделок
    # ушли time-стопом при stop_pct=0.5% — комиссия съела 20% риска на пустом
    # месте). Решаем обратную задачу: находим stop_pct, при котором доля
    # комиссии не превышает потолок.
    fee_min_stop_pct = 2 * CONFIG["FEE_RATE"] / CONFIG["FEE_DRAG_MAX"]
    effective_min_stop = max(CONFIG["MIN_STOP_PCT"], fee_min_stop_pct)

    stop_widened = False
    if stop_pct < effective_min_stop:
        stop_pct = effective_min_stop
        if signal["side"] == "SHORT":
...
def format_message(symbol: str, signal: dict, plan: dict, sid: str) -> str:
    ...
    if plan["stop_widened"]:
        lines.append(f"❗ Стоп расширен до {plan['stop_pct']*100:.2f}% — "
                     "структурный был уже: либо тонул бы в шуме/спреде, либо "
                     "комиссия съедала бы слишком большую долю риска.")
    if plan["capped"]:
