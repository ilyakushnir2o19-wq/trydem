...
CONFIG = {
    ...
    "DAILY_STOP_LIMIT_ALL": 5,     # стопов по ВСЕМ сигналам -> стратегия холодная
    ...
    "IMPULSE_VETO_ATR": 2.0,       # ход > 2×ATR против сделки = вето
...
def analyze(df: pd.DataFrame, trend_1h: str, symbol: str = "?") -> dict | None:
    ...
    if sweep_hi and (wick_up_ok or vol_ok):
        ...
        if wick_up_ok and vol_ok and back_inside and imp <= veto:
        ...
        if imp > veto:
            reasons.append(f"импульс вверх +{imp:.1f} ATR — против свежего "
                           "выброса не шортим (урок SNDK/SOL 09.07)")
    ...
    if sweep_lo and (wick_dn_ok or vol_ok):
        ...
        if wick_dn_ok and vol_ok and back_inside and imp >= -veto:
        ...
        if imp < -veto:
            reasons.append(f"импульс вниз {imp:.1f} ATR — падающий нож не "
                           "ловим (урок ALLO 09.07)")
...
async def update_tracked(symbol: str, df: pd.DataFrame,
    ...
    for trade in STORE.active(symbol):
        ...
        if trade["status"] == "stop":
            ...
            if ((taken_stops >= CONFIG["DAILY_STOP_LIMIT"]
                 or all_stops >= CONFIG["DAILY_STOP_LIMIT_ALL"])
                    and STATE["paused_day"] != day):
                STATE["paused_day"] = day
                reason = (f"{taken_stops} стопа по твоим сделкам"
                          if taken_stops >= CONFIG["DAILY_STOP_LIMIT"]
