...
============================================================================
НОВОЕ В 3.13:
  Добавлен ТРЕНДОВЫЙ СЛОЙ (D1, Turtle 55/20, только LONG выше EMA200):
  вход по пробою максимума 55 дней, стоп 2×ATR, выход по закрытию ниже
  минимума 20 дней. Бэктест 2022-2026: +104R на 10 монетах, но прибыль
  сконцентрирована в редких выездах — риск 0.75%, слой в ФОРВАРД-режиме
  (сигналы без денег). Скальп-ядро ЗАМОРОЖЕНО — без изменений.

НОВОЕ В 3.12 (по разбору журнала 09-13.07, 103 закрытых сделки):
  ПРИЧИНА УБЫТКОВ НАЙДЕНА: гейт FEE_DRAG_MAX=0.08 растягивал 72-89% стопов
  до ровно 1.25%, TP1 уезжал за пределы досягаемости при время-стопе 30 мин.
  До фикса комиссий: +4.7R, винрейт 29%. После: −3.8R, винрейт 7%, 85% time.
  - Стоп НИКОГДА не растягивается: слишком узкий (комиссия >15% риска) или
    слишком широкий (>1.5%, тонкие листинги — винрейт 5%) -> сигнал в ТЕНЬ.
  - FEE_DRAG_MAX 0.08 -> 0.15: минимальный стоп 1.25% -> 0.67%; лучшая
    группа журнала (стопы 0.6-0.9%: +3.7R, винрейт 55%) проходит нетронутой.
  - MIN_SCORE_TO_SEND 8 -> 9: score 8 — худший бакет (n=38, −4.1R, win 8%).
  - SAMSUNG добавлен в EXCLUDE_BASES (токен-акция, просочилась в топ-30).
  - ЗАМОРОЗКА: конфиг не трогаем до ~100 закрытых сделок на этих параметрах.
...
CONFIG = {
    ...
    "EXCLUDE_BASES": {
        ...
        "SNDK", "SKHYNIX", "SAMSUNG", "SOXL", "INTC", "DRAM", "SPCX", "NES", "CHIP",
    ...
    "VOL_STRONG_MULT": 2.2,
    "TREND_LOOKBACK": 3,
    # Найдено 09.07: 73% сигналов (24/33) протухли (stop/time), и ни один
    # старый фильтр (грейд/паттерн/тренд/объём) не объяснял разницу между
    # "доехало" и "протухло" — значит слабое место в САМОМ ЯДРЕ критерия
    # "это ловушка", а не в надстройках сверху. Три ужесточения ниже бьют
    # по этому напрямую:
    "SWEEP_LOOKBACK_SCALP": 48,    # было 20 (100 мин) — слишком локальный,
                                   # мелкий уровень. 48×5m = 4ч — уровень,
                                   # на котором реально скопились стопы.
    "REJECTION_MARGIN_ATR": 0.15,  # закрытие должно быть ЗА уровнем минимум
                                   # на 0.15×ATR, не просто "чуть внутри" —
                                   # отсекает вялые, неубедительные отказы
    "MIN_BODY_ATR": 0.25,          # тело сигнальной свечи >= 0.25×ATR —
                                   # отсекает доджи-с-хвостом без реального
                                   # направленного импульса в отказе
    "HTF_EMA_PERIOD": 200,
    ...
    # Данные 09.07: A=30% доехало до TP1+ vs B=23% (n мало, но направленно
    # совпадает с теорией — больше конфлюэнса важнее). Плюс у B-грейда
    # риск 1.5% и та же абсолютная комиссия — соотношение хуже. Порог поднят
    # с 6 до 8: режем самый слабый хвост, где всё ядро едва наскребло минимум.
    # Данные 13.07 (103 закрытых): score 8 — худший бакет журнала: n=38,
    # −4.1R net, винрейт 8%. Score 9 ≈ ноль, score 10 = +3.5R. Порог 9
    # отрезает именно тот хвост, который тащил статистику вниз.
    "MIN_SCORE_TO_SEND": 9,
    ...
    # Найдено в журнале 09.07: при стопе 0.5% комиссия (вход + 2 частичных
    # выхода ≈ 2x объёма) съедает РОВНО 20% суммы риска ГАРАНТИРОВАННО,
    # ещё до движения цены. У 58% сделок (time-стоп, R=0) это чистый убыток
    # на пустом месте. Минимальный стоп теперь считается от допустимой доли
    # комиссии от риска, а не от фиксированного процента — чиним причину.
    # Комиссия — не плоская. Вход всегда маркет-ордер (taker, "по рынку" из
    # блока КАК ВЗЯТЬ). Выход по TP1/TP2 обычно исполняется резервной лимиткой
    # (maker, дешевле). Выход по стопу/времени — маркетом (taker, дороже).
    # Формула минимального стопа считается по ХУДШЕМУ случаю (полный стоп,
    # taker+taker) — это и есть риск-сценарий, который защищаем.
    "FEE_RATE": 0.0005,            # taker OKX, одна сторона
    "MAKER_FEE_RATE": 0.0002,      # maker OKX, одна сторона (уточни свой тир!)
    # ВСКРЫТИЕ 13.07 (журнал 09-13.07, 103 закрытых): FEE_DRAG_MAX=0.08 давал
    # минимальный стоп 2*0.0005/0.08 = РОВНО 1.25%. Гейт РАСТЯГИВАЛ до него
    # 72-89% стопов, TP1 (=1R) уезжал в 1.25% от входа, а время-стоп оставался
    # 6 свечей (30 мин) — TP1 стал математически недостижим. Итог: до фикса
    # (09.07, структурные стопы) +4.7R и 29% побед; после — −3.8R, 7% побед,
    # 85% time-стопов. Лучшая группа журнала — стопы 0.6-0.9%: +3.7R, 55%.
    # ЛЕЧЕНИЕ: (1) стоп больше НИКОГДА не растягивается — геометрия сетапа
    # священна; (2) допустимое трение поднято до 15% (min stop 0.67% — лучшая
    # группа проходит нетронутой); (3) слишком узкий стоп -> сигнал в ТЕНЬ,
    # /stats покажет, чего это стоило; (4) стоп шире MAX_STOP_PCT (тонкие
    # свежие листинги: EDGE/BEAT/BZ; группа >1.3%: винрейт 5%) -> тоже в тень.
    "FEE_DRAG_MAX": 0.15,          # комиссия ХУДШЕГО случая ≤ 15% суммы риска
    "MIN_STOP_PCT": 0.005,         # абсолютный пол на случай экзотики
    "MAX_STOP_PCT": 0.015,         # шире 1.5% (scalp) = дикий инструмент, в тень

    # ==================== ТРЕНДОВЫЙ СЛОЙ (D1, Turtle 55/20) =============
    # Бэктест 2022-2026 (10 монет): +104R, но 95% прибыли — 5 сделок,
    # серия из 24 убытков подряд была. Поэтому риск МАЛЕНЬКИЙ и не
    # обсуждается. Валидация-2026 слабая (год без трендов) — слой в
    # форвард-режиме: сигналы без денег, копим журнал.
    "TREND_ENABLED": True,
    "TREND_BREAKOUT_N": 55,        # вход: закрытие дня выше максимума N дней
    "TREND_EMA_REGIME": 200,       # только лонги и только выше EMA200
    "TREND_STOP_ATR": 2.0,         # стартовый стоп 2×ATR(20), не двигается
    "TREND_EXIT_N": 20,            # выход: закрытие дня ниже минимума N дней
    "TREND_RISK_PCT": 0.0075,      # 0.75% на сделку. НЕ ПОВЫШАТЬ: см. серию 24
    "TREND_SCAN_EVERY_MIN": 60,    # опрос дневок раз в час на монету
    # Порог "TP1 < $X" заменён на FEE_DRAG_MAX — управляет минимальным стопом
    # напрямую. Оставлен для обратной совместимости, None = выключено.
    "MIN_TP1_NET_USD": None,
    ...
    # Раньше делил один SWEEP_LOOKBACK со scalp-слоем (20 = 20 часов) —
    # уровень был мельче, чем текст сигнала обещал ("~13 дней"). Теперь
    # свой, реально соответствующий заявленному горизонту.
    "SWEEP_LOOKBACK_SWING": 280,   # ~11.7 дней — значимый уровень
    ...
    # --- Соответствие минимальному лоту биржи ---
    # Позиция округляется под шаг/минимум контракта OKX. Если для этого
    # риск пришлось бы поднять больше чем на LOT_BUMP_MAX_RATIO — сигнал
    # блокируется как физически неисполнимый, а не тихо занижается.
    "LOT_BUMP_MAX_RATIO": 1.5,     # объём можно поднять максимум в 1.5x
...
VERSION = "3.13"   # единая версия: приветствие, /start и логи берут отсюда
...
def add_indicators(df: pd.DataFrame, sweep_lookback: int | None = None) -> pd.DataFrame:
    df = df.copy()
    ...
    lb = sweep_lookback or CONFIG["SWEEP_LOOKBACK_SCALP"]
...
def analyze(df: pd.DataFrame, trend_1h: str, symbol: str = "?",
           sweep_lookback: int | None = None) -> dict | None:
    """
    Односвечная ловушка: обязательное ядро (6 баллов) = sweep + хвост >=50% +
    объём >=1.3x + ЧИСТОЕ закрытие за снятым уровнем (маржа REJECTION_MARGIN_ATR,
    не "чуть внутри") + тело свечи >= MIN_BODY_ATR (не доджи). Бонусы: EMA20 +1,
    ...
    sweep_lb = sweep_lookback or CONFIG["SWEEP_LOOKBACK_SCALP"]
    need = sweep_lb + CONFIG["EMA_PERIOD"] + lb + 2
    if len(df) < need:
        return None
    ...
    body_ok = body_size >= CONFIG["MIN_BODY_ATR"] * atr_v if atr_v > 0 else False
    margin = CONFIG["REJECTION_MARGIN_ATR"] * atr_v
    ...
    if sweep_hi and (wick_up_ok or vol_ok):
        back_inside = last["close"] < last["swing_high"] - margin
        if wick_up_ok and vol_ok and back_inside and body_ok:
        ...
        two = _two_candle(df, trend_1h, symbol, sweep_lb)
        if two:
            return two
        reasons = []
        if not wick_up_ok:
        ...
        if not back_inside:
            ...
            reasons.append(f"тело свечи {body_size/atr_v:.2f}×ATR<"
                           f"{CONFIG['MIN_BODY_ATR']}×ATR (доджи, нет напора)")
    ...
    if sweep_lo and (wick_dn_ok or vol_ok):
        back_inside = last["close"] > last["swing_low"] + margin
        if wick_dn_ok and vol_ok and back_inside and body_ok:
    ...
    return _two_candle(df, trend_1h, symbol, sweep_lb)


def _two_candle(df: pd.DataFrame, trend_1h: str, symbol: str,
                sweep_lookback: int | None = None) -> dict | None:
    ...
    sweep_lb = sweep_lookback or CONFIG["SWEEP_LOOKBACK_SCALP"]
    if not CONFIG["TWO_CANDLE_ENABLED"] or len(df) < sweep_lb + 25:
        return None
    ...
    atr_b = float(b["atr"])
    margin_b = CONFIG["REJECTION_MARGIN_ATR"] * atr_b
    body_b_ok = (abs(b["close"] - b["open"]) >= CONFIG["MIN_BODY_ATR"] * atr_b
                if atr_b > 0 else False)

    level_hi = float(a["swing_high"])
    if (a["high"] > level_hi and a["close"] > level_hi
            and b["close"] < level_hi - margin_b and b["close"] < b["open"]
            and vol_pair_ok and imp <= veto and body_b_ok):
    ...
    if (a["low"] < level_lo and a["close"] < level_lo
            and b["close"] > level_lo + margin_b and b["close"] > b["open"]
            and vol_pair_ok and imp >= -veto and body_b_ok):
...
def build_trade_plan(signal: dict, balance: float) -> dict:
    ...
    # Минимальный стоп = max(структурный пол, стоп, при котором комиссия
    # не превышает FEE_DRAG_MAX от суммы риска): fee_to_risk = 2×FEE_RATE /
    # stop_pct. v3.12: стоп, не проходящий гейт, больше НЕ растягивается —
    # растяжение до 1.25% уносило TP1 (=1R) за пределы досягаемости при
    # неизменном время-стопе 30 мин (журнал 09-13.07: 85% time-стопов,
    # винрейт 7% против 29% на нетронутых стопах). Вместо этого сигнал
    # блокируется в тень — signal_blocks() увидит флаги ниже. То же для
    # аномально широкого стопа (тонкие листинги, винрейт 5%): дикий
    # инструмент — не наш сетап. LIFE/swing из верхнего гейта исключены:
    # их широкий стоп (>=2×ATR) — осознанная часть режима раннера.
    fee_min_stop_pct = 2 * CONFIG["FEE_RATE"] / CONFIG["FEE_DRAG_MAX"]
    effective_min_stop = max(CONFIG["MIN_STOP_PCT"], fee_min_stop_pct)
    stop_too_narrow = stop_pct < effective_min_stop
    stop_too_wide = (not life) and stop_pct > CONFIG["MAX_STOP_PCT"]
    ...
    risk_amt = position_usdt * stop_pct

    # Вход всегда маркет (taker) — "по рынку" из блока КАК ВЗЯТЬ.
    # Худший случай — полный стоп/время-стоп: выход тоже маркетом (taker).
    # Лучший случай — TP1/TP2 сработали резервными лимитками (maker).
    # Правило маркер/тейкер: TP-хит = maker, стоп/время-стоп = taker.
    fee_entry = position_usdt * CONFIG["FEE_RATE"]
    fee_worst_usd = fee_entry + position_usdt * CONFIG["FEE_RATE"]        # taker+taker
    fee_best_usd = fee_entry + position_usdt * CONFIG["MAKER_FEE_RATE"]   # taker+maker
    fee_to_risk = fee_worst_usd / risk_amt if risk_amt > 0 else 0.0       # для гейта стопа

    # tp1_net_usd: прибыль на закрытых 50% на TP1, за вычетом их доли комиссии
    # (половина входа taker + сам выход maker)
    tp1_net_usd = (0.5 * position_usdt * stop_pct
                  - 0.5 * position_usdt * CONFIG["FEE_RATE"]
                  - 0.5 * position_usdt * CONFIG["MAKER_FEE_RATE"])
    ...
    return {
        ...
        "stop_too_narrow": stop_too_narrow, "stop_too_wide": stop_too_wide,
        "min_stop_needed": effective_min_stop, "tp1_net_usd": tp1_net_usd,
        "fee_entry": fee_entry, "fee_worst_usd": fee_worst_usd,
        "fee_best_usd": fee_best_usd, "fee_to_risk": fee_to_risk,
        "risk_amt": risk_amt,
...
def signal_blocks(signal: dict, plan: dict) -> str | None:
    ...
    if CONFIG["BLOCK_LIQ_UNSAFE"] and not plan["liq_safe"]:
        return (f"ликвидация {fmt_price(plan['liq'])} ближе стопа "
                ...
                f"{plan['leverage']}x")
    if plan.get("stop_too_narrow"):
        return (f"стоп {plan['stop_pct']*100:.2f}% уже минимума "
                f"{plan['min_stop_needed']*100:.2f}% — комиссия съела бы "
                f">{CONFIG['FEE_DRAG_MAX']*100:.0f}% риска, а растяжение "
                "стопа ломает геометрию (проверено журналом 09-13.07)")
    if plan.get("stop_too_wide"):
        return (f"стоп {plan['stop_pct']*100:.2f}% шире "
                f"{CONFIG['MAX_STOP_PCT']*100:.1f}% — тонкий/дикий "
                "инструмент, сетап не имеет преимущества")
...
def format_message(symbol: str, signal: dict, plan: dict, sid: str) -> str:
    ...
    lines += [
        ...
        f"💸 Комиссия ~{plan['fee_worst_usd']:.2f} USDT ({plan['fee_to_risk']*100:.0f}% "
        f"от риска сделки) — платится независимо от исхода",
...
def make_trade_record(sid: str, symbol: str, signal: dict, plan: dict,
    ...
    return {
        ...
        "risk_amt": round(plan.get("risk_amt", plan["position_usdt"]*plan["stop_pct"]), 4),
        "fee_to_risk": round(plan.get("fee_to_risk", 0.0), 4),
        "result_r_net": None,          # R за вычетом комиссии (см. track_trade)
        # --- 💎 lifechange / раннер ---
    ...
    for _, cndl in new.iterrows():
        ...
        if trade["status"] == "active":
            ...
            if hit_stop:                                   # стоп раньше цели
                ...
                trade["result_r_net"] = round(trade["result_r"] - trade.get("fee_to_risk", 0.0), 3)
...
def _net_r(t: dict) -> float:
    """Чистый R после комиссий, если посчитан; иначе валовый (старые записи)."""
    net = t.get("result_r_net")
    return net if net is not None else t.get("result_r", 0.0)
...
def _stats_block(trades: list[dict], title: str) -> list[str]:
    ...
    wins = sum(1 for t in closed if _net_r(t) > 0)
    total_net = sum(_net_r(t) for t in closed)
    return [f"{title}: {len(closed)} закр., прибыльных (net) "
            f"{wins/len(closed)*100:.0f}%, суммарно {total_net:+.1f}R (net)"]
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if closed:
        ...
        for t in closed:
            by[t["status"]] += 1
        wins = sum(1 for t in closed if _net_r(t) > 0)
        total_gross = sum(t["result_r"] for t in closed)
        total_net = sum(_net_r(t) for t in closed)
        total_usd = sum(_net_r(t) * t.get("risk_amt", 0.0) for t in closed)
        avg_fee_drag = (sum(t.get("fee_to_risk", 0) for t in closed)
                       / len(closed) * 100)
        best = max(closed, key=lambda t: _net_r(t))
        lines += [
            f"🏆 TP2: {by['tp2']} | 💎 Раннер: {by['run']} | ⚖️ БУ: {by['be']} | "
            f"🛑 Стоп: {by['stop']} | ⏰ Время: {by['time']}",
            f"Прибыльных (net): {wins/len(closed)*100:.0f}%",
            f"Итог ВАЛОВЫЙ (до комиссий): {total_gross:+.1f}R",
            f"Итог ЧИСТЫЙ (после комиссий): {total_net:+.1f}R "
            f"≈ {total_usd:+.2f} USDT",
            f"Комиссия съедала в среднем {avg_fee_drag:.0f}% суммы риска "
            f"каждой сделки",
            f"Лучшая: #{best['id']} {_net_r(best):+.2f}R (net)",
            "",
        ]
        for g in ("A+", "A", "B"):
        ...
        if shadow:
            tot = sum(_net_r(t) for t in shadow)
            lines.append(f"\n🕵️ ТЕНЕВОЙ ЖУРНАЛ — что отрезали фильтры "
                         f"({len(shadow)} закрыто, итог {tot:+.1f}R net; "
                         "комиссия в тени не учитывалась, скидка условная):")
            by_reason: dict = {}
            for t in shadow:
...
async def update_tracked(symbol: str, df: pd.DataFrame,
    ...
    for trade in STORE.active(symbol, include_virtual=True):
        if trade.get("layer") == "trend":
            continue                  # трендовый слой ведётся на дневках (scan_trend)
...
async def scan_symbol(exchange: ccxt.okx, htf: HtfCache, funding: FundingCache,
    ...
    if signal["score"] < CONFIG["MIN_SCORE_TO_SEND"]:
        # v3.12: порог поднят до 9 по данным, где score-8 был замешан с
        # растянутыми стопами (конфаундер). Чтобы решение проверило само
        # себя — score-8 сигналы идут в ТЕНЬ: /stats покажет, режет порог
        # прибыль или спасает деньги.
        if signal["score"] == CONFIG["MIN_SCORE_TO_SEND"] - 1:
            plan = build_trade_plan(signal, CONFIG["FALLBACK_BALANCE_USDT"])
            sid = f"V{symbol.split('/')[0]}{sig_ts//60000 % 100000}"
            STORE.add(make_trade_record(sid, symbol, signal, plan, virtual=True,
                                        block_reason="score ниже порога"))
            STATE["blocked_signals"] = STATE.get("blocked_signals", 0) + 1
            runtime["last_sig"][symbol] = sig_ts
        return

    # H4-бонус: сигнал на 5m, совпавший с настоящим уровнем старшего
    # порядка (не мелким 4-часовым локальным экстремумом) — сильнее
    # структурно. Раньше это давало плюс только 💎-кандидатам, теперь
    # общий балл для любого scalp-сигнала.
    h4_note = await h4sweep.get(symbol, signal["side"])
    if h4_note and signal["score"] < 12:
        signal["score"] = min(12, signal["score"] + 1)
        signal["why"] = signal.get("why", []) + [
            "совпадает со снятием уровня на 4h — не локальный шум"]
...
async def scan_swing(exchange: ccxt.okx, h4sweep: H4SweepCache, symbol: str,
    ...
    df1 = add_indicators(await fetch_df(
        ...
        CONFIG["SWING_CANDLES_LIMIT"]), CONFIG["SWEEP_LOOKBACK_SWING"])
    trend4 = await h4sweep.trend(symbol)
    signal = analyze(df1, trend4, symbol, CONFIG["SWEEP_LOOKBACK_SWING"])
...
async def _track_trend(trade: dict, df: pd.DataFrame,
                       app: Application | None) -> None:
    """Ведение трендовой сделки: стоп смотрим по каждому дню (включая
    формирующийся — защита внутри дня), выход по тренду — только по
    ЗАКРЫТОМУ дню ниже минимума TREND_EXIT_N дней."""
    long = trade["side"] == "LONG"
    coin = trade["symbol"].split("/")[0]
    tag = f"{coin} TREND #{trade['id']}"
    closed, forming = df.iloc[:-1], df.iloc[-1]
    lo_exit = closed["low"].shift(1).rolling(CONFIG["TREND_EXIT_N"]).min()

    def close_trade(status: str, exit_px: float, ts: int, msg: str) -> None:
        r = (exit_px - trade["entry"]) / trade["r_unit"]
        if not long:
            r = -r
        trade["status"] = status
        trade["result_r"] = round(r, 3)
        trade["result_r_net"] = round(r - trade.get("fee_to_risk", 0.0), 3)
        trade["closed_day"] = local_day_key(ts)
        trade["last_ts"] = ts

    new = closed[closed["timestamp"] > trade["last_ts"]]
    for idx, cndl in new.iterrows():
        if trade["status"] != "active":
            break
        trade["last_ts"] = int(cndl["timestamp"])
        trade["candles_seen"] += 1
        lo = float(cndl["low"])
        if long and lo <= trade["stop"]:
            close_trade("stop", trade["stop"], int(cndl["timestamp"]),
                        "стоп")
            await send_signal(app, f"🛑 СТОП по тренду: {tag}. −1R "
                              f"(−{trade['risk_pct']*100:.2f}% депо). "
                              "У трендовой системы это норма: большинство "
                              "попыток — мелкий минус, платят редкие выезды.")
            STORE.save()
            return
        ex = lo_exit.loc[idx]
        if pd.notna(ex) and float(cndl["close"]) < float(ex):
            px = float(cndl["close"])
            close_trade("dc", px, int(cndl["timestamp"]), "выход")
            r = trade["result_r"]
            await send_signal(app, f"🏁 ВЫХОД по тренду: {tag}. Закрытие дня "
                              f"ниже минимума {CONFIG['TREND_EXIT_N']} дней — "
                              f"тренд сломан. Выходи по рынку ~{fmt_price(px)}. "
                              f"Итог: {r:+.2f}R "
                              f"({r*trade['risk_pct']*100:+.2f}% депо).")
            STORE.save()
            return
    # защита внутри текущего (незакрытого) дня — только жёсткий стоп
    if trade["status"] == "active" and long and             float(forming["low"]) <= trade["stop"]:
        close_trade("stop", trade["stop"], int(forming["timestamp"]), "стоп")
        await send_signal(app, f"🛑 СТОП по тренду: {tag} (внутри дня). −1R "
                          f"(−{trade['risk_pct']*100:.2f}% депо).")
        STORE.save()


async def scan_trend(exchange: ccxt.okx, symbol: str,
                     app: Application | None, runtime: dict) -> None:
    """Трендовый слой D1: вход — закрытие дня выше максимума
    TREND_BREAKOUT_N дней при цене выше EMA200; стоп 2×ATR; выход —
    закрытие ниже минимума TREND_EXIT_N дней. Только LONG (шорт-тренды
    в крипте бэктест не оправдал). Сигналы редкие и сезонные: месяцы
    тишины в боковике — нормальное состояние слоя."""
    if not CONFIG.get("TREND_ENABLED"):
        return
    now_s = time.time()
    if now_s - runtime["trend_poll"].get(symbol, 0.0) <             CONFIG["TREND_SCAN_EVERY_MIN"] * 60:
        return
    runtime["trend_poll"][symbol] = now_s

    need = CONFIG["TREND_EMA_REGIME"] + CONFIG["TREND_BREAKOUT_N"] + 10
    df = await fetch_df(exchange, symbol, "1d", min(need + 40, 300))
    if len(df) < need:
        return                              # молодой листинг — не для трендовика

    # 1) ведение открытых трендовых сделок (стоп — даже внутри дня)
    for trade in STORE.active(symbol, include_virtual=True):
        if trade.get("layer") == "trend":
            await _track_trend(trade, df, app)

    # 2) новый сигнал — только на последнем ЗАКРЫТОМ дне
    closed = df.iloc[:-1]
    last = closed.iloc[-1]
    sig_ts = int(last["timestamp"])
    key = symbol + "|trend"
    if sig_ts == runtime["last_sig"].get(key, 0):
        return
    runtime["last_sig"][key] = sig_ts
    if any(t.get("layer") == "trend"
           for t in STORE.active(symbol, include_virtual=True)):
        return                              # одна трендовая позиция на монету

    closes = closed["close"]
    ema_r = closes.ewm(span=CONFIG["TREND_EMA_REGIME"], adjust=False,
                       min_periods=CONFIG["TREND_EMA_REGIME"]).mean()
    atr_d = atr(closed["high"], closed["low"], closed["close"],
                CONFIG["ATR_PERIOD"])
    if pd.isna(ema_r.iloc[-1]) or pd.isna(atr_d.iloc[-1]):
        return
    hi_n = float(closed["high"].iloc[-(CONFIG["TREND_BREAKOUT_N"] + 1):-1].max())
    entry = float(last["close"])
    atr_v = float(atr_d.iloc[-1])
    if atr_v <= 0 or entry <= hi_n or entry <= float(ema_r.iloc[-1]):
        return

    stop = entry - CONFIG["TREND_STOP_ATR"] * atr_v
    stop_pct = (entry - stop) / entry
    balance = await fetch_usdt_balance(exchange)
    risk_amt = balance * CONFIG["TREND_RISK_PCT"]
    position = risk_amt / stop_pct if stop_pct > 0 else 0.0
    fee_to_risk = 2 * CONFIG["FEE_RATE"] / stop_pct
    sid = f"T{symbol.split('/')[0]}{sig_ts // 60000 % 100000}"
    coin = symbol.split("/")[0]

    rec = {
        "id": sid, "symbol": symbol, "side": "LONG", "virtual": False,
        "block_reason": None, "layer": "trend", "time_stop_candles": 0,
        "score": 0, "grade": "TREND",
        "pattern": (f"Turtle {CONFIG['TREND_BREAKOUT_N']}/"
                    f"{CONFIG['TREND_EXIT_N']}"),
        "trend_1h": "up", "vol_x": 0.0,
        "signal_ts": sig_ts, "signal_time": fmt_time(last["dt"]),
        "entry": entry, "stop": stop, "tp1": None, "tp2": None,
        "stop_pct": round(stop_pct, 5),
        "risk_pct": CONFIG["TREND_RISK_PCT"],
        "position_usdt": round(position, 2),
        "risk_amt": round(risk_amt, 4),
        "fee_to_risk": round(fee_to_risk, 4),
        "result_r_net": None, "mode": "trend", "lc": [],
        "r_unit": entry - stop, "atr_sig": atr_v,
        "extreme": entry, "trail": None, "trail_notified": entry,
        "taken": None, "status": "active", "result_r": None,
        "candles_seen": 0, "last_ts": sig_ts, "closed_day": None,
        "events": [],
    }
    STORE.add(rec)
    STATE["signals_sent"] += 1
    msg = (f"📈 ТРЕНД-СИГНАЛ [D1] {coin} LONG #{sid}\n\n"
           f"Закрытие дня пробило максимум {CONFIG['TREND_BREAKOUT_N']} дней "
           f"в бычьем режиме (выше EMA200).\n"
           f"Вход: ~{fmt_price(entry)} (по рынку на открытии дня)\n"
           f"Стоп: {fmt_price(stop)} (−{stop_pct*100:.1f}%), НЕ двигается\n"
           f"Риск: {CONFIG['TREND_RISK_PCT']*100:.2f}% депо "
           f"(позиция ~{position:.0f} USDT, без плеча)\n"
           f"Целей нет: держим, пока бот не даст ВЫХОД (закрытие дня ниже "
           f"минимума {CONFIG['TREND_EXIT_N']} дней). Это может занять "
           f"недели — так и задумано.\n\n"
           f"⚠️ Слой в форвард-проверке: сигнал для журнала, не для депозита.")
    log.info("📈 TREND-СИГНАЛ %s LONG #%s", symbol, sid)
    await send_signal(app, msg, reply_markup=signal_keyboard(sid))
    STATE["last_signal_text"] = msg.split("\n\n")[0]
    STORE.save()
        ...
        try:
            ...
            for sym in symbols:
                try:
                    ...
                    await scan_trend(exchange, sym, app, runtime)
